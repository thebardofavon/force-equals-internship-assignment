...
export async function POST(request: NextRequest) {
  try {
    ...
    const seller = await Database.getUserById(sellerId)

    if (!seller || !seller.google_access_token) {
      return NextResponse.json(
        { error: 'Seller not found or not authenticated with Google' },
        { status: 404 }
      )
    }
    ...
    // Set time range for the selected date (full day)
